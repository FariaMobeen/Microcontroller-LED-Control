/*
 * File:   main.c
 * Author: Faria Mobeen, Fabiha Tuheen, Mobha Khan
 *
 */
...
#define ClrWdt() {asm volatile ("clrwdt");}
#define Sleep() {asm volatile ("pwrsav #0");}   // set sleep mode
#define Idle() {asm volatile ("pwrsav #1");}
#define dsen() {asm volatile ("BSET DSCON, #15");}

        
int main(void) {
    //Clock output on REFO pin - pin 12 RB8
    TRISBbits.TRISB8 = 0;  // Set RB8 as output for REFO
    REFOCONbits.ROEN = 1; // Ref oscillator is enabled
    REFOCONbits.ROSSLP = 0; // Ref oscillator is disabled in sleep
    REFOCONbits.ROSEL = 0; // Output base clk showing clock switching
    REFOCONbits.RODIV = 0b0000;
    
    NewClk(32); // Set clock speed to 32 kHz
    IOinit(); // Initialize IO ports
    InitUART2(); //Initialize UART settings and enable UART ports
    
    while(1) { 
        // Check IO and control LED
        StartInterrupt();
    }
    return 0;
}
